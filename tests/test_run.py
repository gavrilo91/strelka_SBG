...
def test_strelka(normal_bam_name: str = 'NA12892_demo20.bam',
    ...
    try:
        normal_bam = list(session.api.files.query(project=TEST_PROJECT,
    ...
    try:
        tumor_bam = list(session.api.files.query(project=TEST_PROJECT,
    ...
    try:
        reference = list(session.api.files.query(project=TEST_PROJECT,
                                                 names=[reference_name]))
        log.warning(" Reference file is here!")
    except IndexError:
        raise Exception('Missing reference file') from None

    session.run(TEST_PROJECT, app, dict(reference_fasta=reference[0],
